...
(
    ...
    from
        ...
        on avdp.province = dvi.province and avdp.vehicleType = dvi.vehicleType
        where avdp.diff_Voltage > round(dvi.vol_max_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join diff_Voltage_info  on  tmp.province = diff_Voltage_info.province and tmp.vehicleType = diff_Voltage_info.vehicleType
),
...
(
    ...
    from
        ...
        on avdp.province = dti.province and avdp.vehicleType = dti.vehicleType
        where avdp.diff_temper > round(dti.diff_temper_max_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join diff_temper_info  on  tmp.province = diff_temper_info.province and tmp.vehicleType = diff_temper_info.vehicleType
),
...
(
    ...
    from
        ...
        on avdp.province = tri.province and avdp.vehicleType = tri.vehicleType
        where avdp.temper_rate > round(tri.temper_rate_max_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join temper_rate_info  on  tmp.province = temper_rate_info.province and tmp.vehicleType = temper_rate_info.vehicleType
),
...
(
    ...
    from
        ...
        on avdp.province = ti.province and avdp.vehicleType = ti.vehicleType
        where avdp.temper > round(ti.temper_max_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join temper_info  on  tmp.province = temper_info.province and tmp.vehicleType = temper_info.vehicleType
),
...
(
    ...
    from
        ...
        on avdp.province = ri.province and avdp.vehicleType = ri.vehicleType
        where avdp.resistance < round(ri.resistance_min_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join resistance_info  on  tmp.province = resistance_info.province and tmp.vehicleType = resistance_info.vehicleType
),
...
(
    ...
    from
        ...
        on avdp.province = wi.province and avdp.vehicleType = wi.vehicleType
        where avdp.wDischargeRate > round(wi.wDischargeRate_max_value,1)
        group by avdp.province,avdp.vehicleType)  tmp
    join wDischargeRate_info  on  tmp.province = wDischargeRate_info.province and tmp.vehicleType = wDischargeRate_info.vehicleType
)
...
join  temper_rate_statistics    trs on  dvs.province = trs.province and dvs.vehicleType = trs.vehicleType
join  temper_statistics         ts  on  dvs.province = ts.province and dvs.vehicleType = ts.vehicleType
join  diff_temper_statistics    dts on  dvs.province = dts.province and dvs.vehicleType = dts.vehicleType
join  resistance_statistics     rs  on  dvs.province = rs.province and dvs.vehicleType = rs.vehicleType
join  wDischargeRate_statistics wds on  dvs.province = wds.province and dvs.vehicleType = wds.vehicleType;
